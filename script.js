...
function actualizarListaMaterialesReceta() {
    ...
    const html = window.recetaMateriales.map(item => {
        ...
        const texto = cantidad >= 1
            ? `${cantidad.toFixed(2)} jugos por unidad`
            : `${cantidad.toFixed(2)} por jugo`;
        return `<li>${nombre}: ${texto}</li>`;
    }).join('');

    lista.innerHTML = html;
}

function obtenerRecetaPorId(recetaId) {
    const lista = normalizarLista(window.recetasCache || [], ['recetas']);
    if (!recetaId) return null;
    return lista.find(r => String(r.id) === String(recetaId) || String(r.receta_id) === String(recetaId)) || null;
}

async function actualizarInventarioPorVenta(recetaId, cantidadVendida) {
    if (!recetaId || !cantidadVendida || Number(cantidadVendida) <= 0) return;

    try {
        const recetas = await llamarAPI('obtenerRecetas');
        const listRecetas = normalizarLista(recetas, ['recetas']);
        const receta = listRecetas.find(r => String(r.id) === String(recetaId) || String(r.receta_id) === String(recetaId));
        ...
        const stockActual = normalizarLista(inventario, ['inventario']);

        const nuevoInventario = stockActual.map(item => {
            const nombre = String(item.nombre || item.material_nombre || '').toLowerCase();
            const materialId = item.id ?? item.material_id;
            const consumo = detalles.reduce((total, detalle) => {
                const detalleId = detalle.material_id ?? detalle.materialId ?? detalle.id;
                const nombreDetalle = String(detalle.material_nombre || detalle.nombre || '').toLowerCase();
                const coincide = String(detalleId) === String(materialId) || nombre.includes(nombreDetalle) || nombreDetalle.includes(nombre);
                if (!coincide) return total;

                const cantidadMaterial = Number(detalle.cantidad ?? detalle.cant ?? 0);
                if (cantidadMaterial <= 0) return total;

                const consumoPorUnidad = cantidadMaterial >= 1
                    ? Number(cantidadVendida) / cantidadMaterial
                    : Number(cantidadVendida) * cantidadMaterial;

                return total + consumoPorUnidad;
            }, 0);

            if (consumo <= 0) return item;

            const stock = Number(item.stock_actual ?? item.stockActual ?? 0);
            return { ...item, stock_actual: Math.max(0, stock - consumo) };
        });

        window.inventarioCache = nuevoInventario;
        ...
        console.warn('No se pudo ajustar el inventario local:', e);
    }
}
...
async function registrarVenta() {
    ...
    try {
        ...
        if (resultado && resultado.success) {
            ...
            await actualizarInventarioPorVenta(recetaId, cantidad);
